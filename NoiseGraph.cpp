...
NoiseGraph::NoiseGraph(unsigned dimension) : noiseGenSystem( LSystem('A' + rand() % 11, 1)), nullFloat(0.0f)
{
...
{
	if (nullFloat == 0 && noiseGenSystem.isFinished())
		nullFloat = getNoise(0, 0) * 5;
	return (getNoise(x/20, y/20) * 5) - nullFloat;
}
