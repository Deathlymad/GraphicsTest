...
PointLight::PointLight( vec3 c, float i,Attenuation a, vec3 p, string& name) : BaseLight( c, i, name), pos( name + ".pos", 3), range( name + ".range", 3), atten(a)
{
	_pos = p;
	calcRange();
	if (shader)
...
PointLight::PointLight(vec3 c, float i, float ex, float lin, float con, vec3 p, string& name) : BaseLight(c, i, name), pos(name + ".pos", 3), range(name + ".range", 3), atten(con, lin, ex, name + ".atten")
{
	_pos = p;
	calcRange();
	if (shader)
...
void SpotLight::render(Shader * s, RenderingEngine::RenderState firstPass)
{
	if (s == shader)
	{
		direction.update((float*)&_direction);
